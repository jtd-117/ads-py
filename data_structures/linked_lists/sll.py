...
class SLL(object):
    ...
    class Node(object):
        ...
        def next(self, new_next):
            ...
            # STEP 1: Ensure the `new_next` is of TYPE `SLL.Node` or `None`
            if (isinstance(new_next, SLL.Node) or (new_next is None)):
                self._next = new_next
                return
            
            # STEP 2: `new_next` is an INAPPROPRIATE type
            raise TypeError("`new_next` must be of TYPE `SLL.Node` or `None`")
    ...
    def cmp_fn(self):
        """
        A custom function for COMPARING `SLL.Node` keys.
    ...
    def head(self, new_head):
        ...
        # STEP 1: Ensure the `new_head` is of TYPE `SLL.Node` or `None`
        if (isinstance(new_head, SLL.Node) or (new_head is None)):
            self._head = new_head
            return
        
        # STEP 2: `new_head` is an INAPPROPRIATE type
        raise TypeError("`new_head` must be of TYPE `SLL.Node` or `None`")
    ...
    def tail(self, new_tail):
        ...
        # STEP 1: Ensure the `new_tail` is of TYPE `SLL.Node` or `None`
        if (isinstance(new_tail, SLL.Node) or (new_tail is None)):
            self._tail = new_tail
            return
        
        # STEP 2: `new_tail` is an INAPPROPRIATE type
        raise TypeError("`new_tail` must be of TYPE `SLL.Node` or `None`")
    ...
    def __iterative_search(self, target_key):
        ...
        while (curr):
            ...
            if (self.cmp_fn(curr.key, target_key) == SLL.CMPValues.EQUAL.value):
                return curr
    ...
    def __recursive_search(self, target_key, self_head):
        ...
        if (self.cmp_fn(self_head.key, target_key) == SLL.CMPValues.EQUAL.value):
            return self_head
