...
class DLL(object):
    ...
    class Node(object):
        ...
        def next(self, new_next):
            ...
            # STEP 1: Ensure the `new_next` is of type `DLL.Node` or `None`
            if (isinstance(new_next, DLL.Node) or (new_next is None)):
                self._next = new_next
                return
            
            # STEP 2: `new_next` is an INAPPROPRIATE type
            raise TypeError("`new_next` must be of TYPE `DLL.Node` or `None`")
        ...
        def prev(self, new_prev):
            ...
            # STEP 1: Ensure the `new_prev` is type `DLL.Node` or `None`
            if (isinstance(new_prev, DLL.Node) or (new_prev is None)):
                self._prev = new_prev
                return
            
            # STEP 2: `new_prev` is an INAPPROPRIATE type
            raise TypeError("`new_prev` must be of TYPE `DLL.Node` or `None`")
    ...
    def cmp_fn(self):
        """
        A custom function for COMPARING `DLL.Node` keys.
    ...
    def head(self, new_head):
        ...
        # STEP 1: Ensure the `new_head` is of TYPE `DLL.Node` or `None`
        if (isinstance(new_head, DLL.Node) or (new_head is None)):
            self._head = new_head
            return
        
        # STEP 2: `new_head` is an INAPPROPRIATE type
        raise TypeError("`new_head` must be of TYPE `DLL.Node` or `None`")
    ...
    def tail(self, new_tail):
        ...
        # STEP 1: Ensure the `new_tail` is of TYPE `DLL.Node` or `None`
        if (isinstance(new_tail, DLL.Node) or (new_tail is None)):
            self._tail = new_tail
            return
        
        # STEP 2: `new_tail` is an inappropriate type
        raise TypeError("`new_tail` must be of TYPE `DLL.Node` or `None`")
    ...
    def __iterative_search(self, target_key):
        ...
        while (curr):
            ...
            if (self.cmp_fn(curr.key, target_key) == DLL.CMPValues.EQUAL.value):
                return curr
    ...
    def __recursive_search(self, target_key, self_head):
        ...
        if (self.cmp_fn(self_head.key, target_key) == DLL.CMPValues.EQUAL.value):
            return self_head
